...
class GameGUI:
    ...
    def draw_board(self):
        # Fondo del tablero
        board_rect = pygame.Rect(BOARD_OFFSET_X - 5, BOARD_OFFSET_Y - 5, BOARD_WIDTH + 10, BOARD_HEIGHT + 10)
        ...
        for i in range(1, BOARD_SIZE):
            ...
            pygame.draw.line(self.screen, BLACK, (x_pos, BOARD_OFFSET_Y), 
                           (x_pos, BOARD_OFFSET_Y + BOARD_HEIGHT), line_width)
            y_pos = BOARD_OFFSET_Y + i * CELL_SIZE
            pygame.draw.line(self.screen, BLACK, (BOARD_OFFSET_X, y_pos),
        ...
        # Título
        title = font_title.render("TRES EN RAYA - Q-LEARNING", True, ACCENT_YELLOW)
        self.screen.blit(title, (WINDOW_WIDTH//2 - title.get_width()//2, 30))
        
        # Subtítulo
        subtitle = font_medium.render("Humano (X) vs IA (O)", True, WHITE)
        self.screen.blit(subtitle, (WINDOW_WIDTH//2 - subtitle.get_width()//2, 85))
    ...
    def draw_game_status(self):
        ...
        self.screen.blit(status, (WINDOW_WIDTH//2 - status.get_width()//2, status_y))
        
        # Mostrar movimientos
        moves_text = font_small.render(f"Movimiento: {self.state.moves_made}/9", True, WHITE)
        self.screen.blit(moves_text, (WINDOW_WIDTH//2 - moves_text.get_width()//2, status_y + 30))
    ...
    def draw_stats(self):
        ...
        self.screen.blit(stats_title, (stats_x + (stats_width - stats_title.get_width())//2, stats_y + 10))
        
        # Línea separadora
        pygame.draw.line(self.screen, ACCENT_YELLOW, 
    ...
    def draw_buttons(self):
        """Dibuja botón centrado"""
        mouse_pos = pygame.mouse.get_pos()
        
        # Botón Nuevo Juego CENTRADO
        button_color = (30, 160, 80) if self.new_game_button.collidepoint(mouse_pos) else (30, 140, 70)
        pygame.draw.rect(self.screen, button_color, self.new_game_button, border_radius=6)
        
        text = font_small.render("NUEVO JUEGO", True, WHITE)
        self.screen.blit(text, (self.new_game_button.centerx - text.get_width()//2, 
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if not self.state.game_over and not self.state.ia_thinking and self.state.current_player == 'X':
            ...
            if not self.state.game_over and self.state.current_player == 'O' and self.state.ia_thinking:
                ...
                best_move = self.state.agent.get_best_move(self.state.board)
